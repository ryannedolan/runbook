...
PointAccumulationTitle fastCATTier(int n) => PointAccumulationTitle(
      ...
      description:
          n == 1 ? 'FastCAT — 1000 FastCAT points.' : 'FastCAT $n — ${n * 1000} cumulative points.',
      sport: 'AKC FastCAT',
      sportFilter: Sport.fastCAT,
      pointsNeeded: n * 1000,
    );
...
List<AchievementResult> emitFastCATTiers(List<Q> qs) {
  ...
  final out = <AchievementResult>[
    _bcat().evaluate(qs),
    _dcat().evaluate(qs),
    fastCATTier(1).evaluate(qs),
  ];
  for (var n = 2; n <= fcatTiersUnlocked + 1; n++) {
    out.add(fastCATTier(n).evaluate(qs));
  }
  return out;
}
...
RuleNode akcFastCATTree() => RuleNode.gated(
      ...
      child: RuleNode.group(
        title: 'FastCAT',
        children: [RuleNode.dynamic(emit: emitFastCATTiers)],
      ),
    );
