...
/// Emit MACH / PACH / PAX tiers up to (unlocked + 1). Honors point
/// and QQ overrides so a user who's recorded counts off the AKC
/// report (without yet logging the Qs) sees the matching tier.
List<AchievementResult> emitChampionTiers(
    List<Q> qs, ChampionFamily family, Map<String, int> overrides) {
  ...
  final realDqs = dayMap.values
      .where((cs) =>
          cs.contains(AgilityClass.standard) && cs.contains(AgilityClass.jww))
      .length;

  final pointsOverride =
      family._pointsPerLevel == 0 ? 0 : (overrides[preferred ? 'points::pach' : 'points::mach'] ?? 0);
  final dqOverride = overrides[preferred ? 'dq::pref' : 'dq::reg'] ?? 0;
  final totalPoints = realPoints > pointsOverride ? realPoints : pointsOverride;
  final doubleQs = realDqs > dqOverride ? realDqs : dqOverride;

  if (totalPoints == 0 && doubleQs == 0) return const [];
  ...
  return [
    for (var n = 1; n <= tiersUnlocked + 1; n++)
      championTier(family, n).evaluate(qs, overrides: overrides),
  ];
}
...
List<AchievementResult> emitT2bTiers(
  ...
  final realCount = qs
      ...
      .length;
  final poolKey = 'agility::t2b::master::${preferred ? "pref" : "reg"}';
  final override = overrides[poolKey] ?? 0;
  final qsCount = realCount > override ? realCount : override;
  if (qsCount == 0) return const [];
  final tiersUnlocked = qsCount ~/ 15;
  return [
    for (var n = 1; n <= tiersUnlocked + 1; n++)
      t2bTier(n, preferred: preferred).evaluate(qs, overrides: overrides),
  ];
}

/// Emit a sequential tier chain — stop after the first tier that's
/// not over the line. Used for bronze/silver/gold/century chains
/// (MX→MXB→MXS→MXG→MXC) and numbered Preferred series
/// (MXP→MXP2→…→MXP5), so the feed doesn't surface MXG before MXB is
/// earned. A pool override (from the AKC points-progression report)
/// counts the same as real Qs for emission, so MXS gets evaluated
/// when the user adjusts the Master Std count past 50.
List<AchievementResult> emitTieredChain(
    List<Q> qs, List<Achievement> tiers, Map<String, int> overrides) {
  final out = <AchievementResult>[];
  for (final t in tiers) {
    final r = t.evaluate(qs, overrides: overrides);
    out.add(r);
    if (r.have < r.need) break;
  }
  return out;
}

// Sub-ranges of the static chains for use as dynamic emitters in the
// rule tree. The full chains stay defined above so the detail page's
// "chain navigation" still shows the complete progression.

List<Achievement> get _regularStandardEarlyTitles =>
    regularStandardChain.titles.sublist(0, 3); // NA, OA, AX
List<Achievement> get _regularStandardMasterTiers =>
    regularStandardChain.titles.sublist(3); // MX/MXB/MXS/MXG/MXC

List<Achievement> get _regularJwwEarlyTitles =>
    regularJwwChain.titles.sublist(0, 3); // NAJ, OAJ, AXJ
List<Achievement> get _regularJwwMasterTiers =>
    regularJwwChain.titles.sublist(3); // MXJ/MJB/MJS/MJG/MJC

List<Achievement> get _regularFastEarlyTitles =>
    regularFastChain.titles.sublist(0, 3); // NF, OF, XF
List<Achievement> get _regularFastMasterTiers =>
    regularFastChain.titles.sublist(3); // MXF/MFB/MFS/MFG/MFC

List<Achievement> get _preferredFastEarlyTitles =>
    preferredFastChain.titles.sublist(0, 3); // NFP, OFP, XFP
List<Achievement> get _preferredFastMasterTiers =>
    preferredFastChain.titles.sublist(3); // MFP/MFPB/MFPS/MFPG/MFPC

// Preferred Std splits into two parallel master ladders.
List<Achievement> get _preferredStandardEarlyTitles =>
    preferredStandardChain.titles.sublist(0, 3); // NAP, OAP, AXP
List<Achievement> get _preferredStandardNumberedTiers =>
    preferredStandardChain.titles.sublist(3, 8); // MXP, MXP2..5
List<Achievement> get _preferredStandardBronzeTiers =>
    preferredStandardChain.titles.sublist(8); // MXPB/MXPS/MXPG/MXPC

List<Achievement> get _preferredJwwEarlyTitles =>
    preferredJwwChain.titles.sublist(0, 3); // NJP, OJP, AJP
List<Achievement> get _preferredJwwNumberedTiers =>
    preferredJwwChain.titles.sublist(3, 8); // MJP, MJP2..5
List<Achievement> get _preferredJwwBronzeTiers =>
    preferredJwwChain.titles.sublist(8); // MJPB/MJPS/MJPG/MJPC
...
TitleProgression _championChainFor(ChampionTitle opened, List<Q>? qs) {
  ...
  if (qs != null) {
    final emitted = emitChampionTiers(qs, family, const {});
...
RuleNode akcAgilityTree() => RuleNode.group(
      title: 'AKC Agility',
      children: [
        // Regular Std + JWW. Early levels (NA/OA/AX, NAJ/OAJ/AXJ) are
        // leaves — they're gated by level, so MJP doesn't show until
        // the dog runs Excellent. The Master tiered chain
        // (MX→MXB→MXS→MXG→MXC, MXJ→MJB→MJS→MJG→MJC) is emitted
        // dynamically: only show the next tier once the previous one
        // is earned.
        for (final t in _regularStandardEarlyTitles) RuleNode.leaf(t),
        RuleNode.dynamic(
          emit: (qs, ov) =>
              emitTieredChain(qs, _regularStandardMasterTiers, ov),
        ),
        for (final t in _regularJwwEarlyTitles) RuleNode.leaf(t),
        RuleNode.dynamic(
          emit: (qs, ov) => emitTieredChain(qs, _regularJwwMasterTiers, ov),
        ),

        // FAST — only evaluated if there's at least one FAST Q.
        // Master tiers are sequential too (MXF→MFB→MFS→MFG→MFC).
        RuleNode.gated(
          gate: (qs) => qs.any((q) => q.agilityClass == AgilityClass.fast),
          child: RuleNode.group(title: 'FAST', children: [
            for (final t in _regularFastEarlyTitles) RuleNode.leaf(t),
            RuleNode.dynamic(
              emit: (qs, ov) =>
                  emitTieredChain(qs, _regularFastMasterTiers, ov),
            ),
            for (final t in _preferredFastEarlyTitles) RuleNode.leaf(t),
            RuleNode.dynamic(
              emit: (qs, ov) =>
                  emitTieredChain(qs, _preferredFastMasterTiers, ov),
            ),
          ]),
        ),
        ...
        RuleNode.gated(
          ...
          child: RuleNode.group(title: 'T2B', children: [
            RuleNode.dynamic(
              emit: (qs, ov) =>
                  emitT2bTiers(qs, preferred: false, overrides: ov),
            ),
            RuleNode.dynamic(
              emit: (qs, ov) =>
                  emitT2bTiers(qs, preferred: true, overrides: ov),
            ),
          ]),
        ),

        // Preferred Std + JWW — only evaluated if there's at least one
        // preferred Q. Two parallel master ladders run on the same
        // pool: numbered (MXP/MXP2/.../MXP5) and bronze
        // (MXPB/MXPS/MXPG/MXPC). Both emit only the next unearned tier.
        RuleNode.gated(
          gate: (qs) => qs.any((q) => q.preferred),
          child: RuleNode.group(title: 'Preferred', children: [
            for (final t in _preferredStandardEarlyTitles) RuleNode.leaf(t),
            RuleNode.dynamic(
              emit: (qs, ov) =>
                  emitTieredChain(qs, _preferredStandardNumberedTiers, ov),
            ),
            RuleNode.dynamic(
              emit: (qs, ov) =>
                  emitTieredChain(qs, _preferredStandardBronzeTiers, ov),
            ),
            for (final t in _preferredJwwEarlyTitles) RuleNode.leaf(t),
            RuleNode.dynamic(
              emit: (qs, ov) =>
                  emitTieredChain(qs, _preferredJwwNumberedTiers, ov),
            ),
            RuleNode.dynamic(
              emit: (qs, ov) =>
                  emitTieredChain(qs, _preferredJwwBronzeTiers, ov),
            ),
          ]),
        ),
        ...
        RuleNode.gated(
          gate: (qs) => qs.any((q) => q.agilityClass?.isPremier ?? false),
        ...
        RuleNode.gated(
          ...
          child: RuleNode.group(
            ...
            children: [
              RuleNode.dynamic(
                emit: (qs, ov) =>
                    emitChampionTiers(qs, ChampionFamily.mach, ov),
              ),
        ...
        RuleNode.gated(
          ...
          child: RuleNode.group(
            ...
            children: [
              RuleNode.dynamic(
                emit: (qs, ov) =>
                    emitChampionTiers(qs, ChampionFamily.pax, ov),
              ),
              RuleNode.dynamic(
                emit: (qs, ov) =>
                    emitChampionTiers(qs, ChampionFamily.pach, ov),
              ),
