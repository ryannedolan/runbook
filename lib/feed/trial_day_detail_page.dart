      ...
      builder: (context, _) {
        ...
        final qs = repo.qsForDog(dog.id).where((q) => _sameDay(q.date, date)).toList()
          ..sort((a, b) {
            ...
            if (s != 0) return s;
            return (a.agilityClass?.index ?? -1)
                .compareTo(b.agilityClass?.index ?? -1);
          });
...
class _QDetailTile extends StatelessWidget {
  ...
  String _classLabel() => switch (q.sport) {
        Sport.akcAgility => () {
            final cls = q.agilityClass;
            if (cls == null) return '—';
            final lvl =
                cls.isSingleLevel ? '' : '${q.level?.label ?? ''} ';
            final pref = q.preferred ? ' Preferred' : '';
            return '$lvl${cls.label}$pref';
          }(),
  ...
  List<({String label, String value})> _stats() {
    ...
    if (q.sct != null) {
      out.add((label: 'SCT', value: '${q.sct!.toStringAsFixed(0)}s'));
    }
    if (q.yards != null) {
      out.add((
        label: 'Yards',
